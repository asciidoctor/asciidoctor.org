...
:customthemeref: link:build-custom-theme-from-asciidoctor-factory#creating-a-new-theme
...
Once you have written your document, it's time to convert it to the format of your choice.
...
First, create an AsciiDoc file like the one below, and name it +sample.adoc+.
...
Remember, Asciidoctor's default backend is +html5+, and therefore it isn't necessary to specifiy it with the +-b+ command. 
...
When you generate a document with the +html5+ backend, the +asciidoctor.css+ stylesheet will be applied by default.
Just browse to +sample.html+ in your browser and checkout the result!

Also note that a copy of the +asciidoctor.css+ file is automatically moved to the same directory as +sample.html+ so that the generated document can link to it.
To embed the stylesheet into the document instead of linking to it, render the document with the following attribute:
...
One of Asciidoctor's strengths is the ease in which you can swap the default stylesheet for alternative Asciidoctor themes or your own custom stylesheet.
...
If you create and use your own stylesheet:
...
If you store your stylesheet in the +stylesheets+ directory, then you need to tell Asciidoctor where to look for it using the +stylesdir+ attribute:
...
You can also add a variety of other resources to the HTML using attributes.

// TODO: expand this section 

If you want to activate syntax highlighting options in the code, add this argument:

 -a source-highlighter=highlightjs 

If you have image references in your document, you'll have to move those with the output document.
Or you can embed images into the document by passing the +data-uri+ attribute to the processor:
...
If you include the attributes in the document's header, you will not need to pass them through the processor when you render the document.
...
== Converting a document to DocBook with Asciidoctor
...
If you're on Linux, you can view the DocBook file with {yelplink}[Yelp].

 $> yelp sample.xml

Using the Asciidoctor Ruby API, so you can generate a DocBook document directly from a Ruby application:
...
There's really no end to the customization you can do to the output that the AsciiDoctor and AsciiDoc processors generate.

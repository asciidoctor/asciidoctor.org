...
However, it makes XML parsers sad (read as: icon:bomb[]).

To accomodate tools that use XML parsers to read the generated HTML, Asciidoctor now supports the XHTML variant of HTML5, known as {uri-xhtml5}[XHTML5].
...
or using a two-line document title:

 Document Title
 ==============

Compat mode is used to enable legacy syntax when the new syntax deviates from it.
See the {uri-migrate}[migration guide] for more information about migrating your AsciiDoc documents and using the transitional syntax.
...
We also made additional refinements to the default stylesheet that bring a professional, modern appearance.

...
:experimental:
:mdash: &#8212;
:ascii-ref: http://asciidoc.org
:what-asciidoc-ref: link:/docs/what-is-asciidoc-why-use-it
:quick-ref: link:/docs/asciidoc-quick-reference/
:ascii-editor-ref: http://asciidoc.org/#_editor_support
:live-ref: link:/docs/editing-asciidoc-with-live-preview/
:ascii-guide-ref: http://asciidoc.org/userguide.html
:ascii-download-ref: http://sourceforge.net/projects/asciidoc/files/latest/download
:ascii-install-ref: http://asciidoc.org/INSTALL.html
...
:ascii-faq-ref: http://asciidoc.org/faq.html
:ascii-mail-ref: http://groups.google.com/group/asciidoc
...
This guide provides a gentle introduction to {ascii-ref}[AsciiDoc], a +plain+ +text+ documentation *syntax* and *processor*.
...
If you're looking for a concise survey of the AsciiDoc syntax, check out the {quick-ref}[AsciiDoc Syntax Quick Reference].
...
The key feature all these editors share is {ascii-editor-ref}[syntax highlighting for AsciiDoc].

TIP: Previewing the output of the document while editing can be helpful.
To learn how to setup instant preview, check out the {live-ref}[Editing AsciiDoc with Live Preview] tutorial.
...
Consult the {ascii-guide-ref}#X98[Audio and video block macros] section of the AsciiDoc User Guide for details.
...
For a complete list of which attributes can be assigned to control the output, consult the {ascii-guide-ref}#X88[Backend Attributes] chapter of the AsciiDoc User Guide.
To see which intrinsic attributes are available, consult the {ascii-guide-ref}#X60[Intrinsic Attributes] chapter.
...
// Is this still right or did 0.1.3 add an override?
...
// Does any of the numbering stuff change with 0.1.3?
...
It is called the preamble, and you don't need to use any syntax to designate this text.
...
Consult the {ascii-guide-ref}#X104[Delimited Blocks] chapter in the AsciiDoc User Guide for a full list of the metadata that is applicable for each block.
...
You can use {ascii-guide-ref}#_conditional_inclusion_macros[conditional inclusion macros] to declare passthrough markup for each of the backends you nee to support.
...
Consult the {ascii-guide-ref}#_tables[Tables] chapter of the AsciiDoc User Guide for a full list.
...
Consult the {ascii-guide-ref}[AsciiDoc User Guide] to continue exploring the AsciiDoc syntax and processor.
...
. {ascii-download-ref}[Download the distribution archive] from the AsciiDoc website.
...
For more details on the manual install, see the {ascii-install-ref}[AsciiDoc Installation Guide].
...
See the {ascii-guide-ref}#X99[Themes] chapter in the AsciiDoc User Guide to learn how to create your own theme.
...
If you want to start tweaking the output that is generated, refer to instructions in the {ascii-guide-ref}[AsciiDoc User Guide].
...
If you get stuck working with the syntax, be sure to browse the {ascii-guide-ref}#_gotchas[Gotchas] section of the AsciiDoc User Guide and the {ascii-faq-ref}[AsciiDoc FAQ] to find answers.
When all else fails, send your question to the {ascii-mail-ref}[AsciiDoc discussion list].

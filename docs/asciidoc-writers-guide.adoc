...
[TIP]
The xref inline macro by default resolves the link to HTML, to create a relative link to an asciidoc file in another subdirectory include 
`ifdef::env-github[:outfilesuffix: .adoc]` below the document header and drop the ".adoc" extension after the filename. 

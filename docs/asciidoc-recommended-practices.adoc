...
See the <<migration.adoc#,migration guide>> for details.
...
You can break any <<user-manual.adoc#splitting-attribute-values-over-multiple-lines,attribute value across several lines>> by ending the lines in a `{backslash}` preceded by a space.

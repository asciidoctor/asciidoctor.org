...
In order to use definition lists inside ordered or unordered lists you must use an \{empty} attribute after the ordered or unordered list marker then place the two portions of the definition list on the next line as follows:

[source]
----
include::ex-dlist.adoc[tag=inner-dlist]
----

Here's how the list is rendered:

.A hybrid list with an inner definition list
====
include::ex-dlist.adoc[tag=inner-dlist]
====

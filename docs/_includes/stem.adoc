...
For Asciimath support, please install the asciimath gem:
...
Also, for full functionality you will need at least asciidoctor version 1.5.4.
...
[NOTE]
====
{uri-org}/asciidoctor-fopub[Asciidoc-fopub] is configured to use the JEuclid FOP plugin. When fopub generates the PDF from DocBook, this plugin interprets the MathML that the asciimath gem added to the file. As a result, AsciiMath stem expressions in the AsciiDoc file will render as expected in the generated PDF.
====

...
In the meantime, it's still possible to apply a style to a column when using the `autowidth` option, as the following attibute list shows:
...
Asciidoctor table cells can contain any Asciidoc content, even another table, aka a nested table.

To distinguish the inner table from the outer one, use `!===` as its table delimiter, and `!` as its cell separator.
Finally make sure the table cell or column is formated with `a` so that it is interpreted as Asciidoc markup.

The following example has a nested table in the last cell.
Notice that the nested table has its own format, independant of that of the outer table:

----
include::ex-table.adoc[tags=nested]
----
...
* Nested tables should be very rare.
There is usually a better way to present the information.

* Check that it works with your output format.
Currently (v1.5.6.1) it works in html but not pdf.
Nested tables are not valid in DocBook 5.0, but FOP processes them anyway.

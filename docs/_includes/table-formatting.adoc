...
To set it to a fixed width, set the `width` attribute in the table's attribute list.
...
You can also set the width based on the content (fit to content) by setting the `autowidth` option.
...
.Result: Table is sized to fit the longest content in each column
[%autowidth]
include::ex-table.adoc[tags=base-h]

[NOTE]
====
`width` takes precedence over `autowidth` if both are set.
`width` works in the html and docbook backends, `autowidth` only works in the html backend.
====

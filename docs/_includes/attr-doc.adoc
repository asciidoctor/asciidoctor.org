...
An [.term]_attribute entry_ is the primary mechanism for defining an attribute in an AsciiDoc document.
Since an attribute acts much like a global variable (and sometimes feature toggle), you can think of an attribute entry as a variable assignment for AsciiDoc.

An attribute entry consists of two parts: an attribute name and the attribute's value.
The attribute's name comes first.

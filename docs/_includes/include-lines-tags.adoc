...
Using the semi-colon as the data separator relieves this requirement.
...
* `**` -- selects all the lines in the document; mimics the default behavior of the include directive.

* `*` -- selects all tagged regions of the document.
...
* `!foo` -- equivalent to `**;!*`; selects non-tagged regions only.

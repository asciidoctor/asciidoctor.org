...
quotation marks.
However, by creating a set of backticks (`{backtick}`) contained within a set of single or double quotes (`{apos}`, `{quot}`), you can tell Asciidoctor where to output curved quotation marks.
...
<2> To output single curved qoutes, enclose a word or phrase in a set of single quotes (`{apos}`) and a set of backticks (`{backtick}`). In this example, the phrase _wormwood and licorice_ should be enclosed in curved single quotes when the document is rendered.
...
To render an apostrophe that is not bound by two characters as curved, mark it as you would a single curved quote.

...
The implicit author line was not intented to support attribute references.
It's best used as a shorthand for defining the author and email.
While attribute references do get replaced in the author line (as part of the header substitution group), they aren't substituted until _after_ the line is parsed, which can produce undesirable and sometimes surprising results.

If you want to use attribute references for the author or email, you should revert to defining the attributes explicitly using attribute entries.

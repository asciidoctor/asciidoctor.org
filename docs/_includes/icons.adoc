...
Icons are used for admonitions and callouts, as well as inserted inline.
{adr} provides 2 ways to display icons: images or fonts.

Use images if you are using the DocBook backend, or want offline output.
The default stylesheet includes images for the admonition and callout icons, which you can replace with your own custom icons.
You can also add any icons you want to display inline.

Alternatively, {adr} can "`draw`" icons using the {fontawesome-ref}[Font Awesome^] font-based icon set.
...
IMPORTANT: The default stylesheet (or any stylesheet produced by the {factory-ref}[Asciidoctor stylesheet factory]) is required when using font based icons.

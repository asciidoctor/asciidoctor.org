...
TIP: Your Fedora system may be configured to automatically update packages, in which case no further action is required by you. Refer to the http://docs.fedoraproject.org[Fedora docs] if you are unsure.

NOTE: The +yum+ package update will not be available at the same time as the +gem+, it may take several weeks before it becomes available. If you must have the latest version now, then use the +gem+ method instead.

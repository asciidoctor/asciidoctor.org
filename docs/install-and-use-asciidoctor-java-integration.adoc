...
// AS or DA: Are there plans to publish it to Jcentral as well?
...
[cols="2,4,^1,5"]
.Artifact information for AsciidoctorJ in Bintray
|===
|Group Id |Artifact Id |Version |Download
...
// AS or DA: Why do we need to clarify that it is based on Asciidoctor? Is there a reason/conflict that requires this clarification?
...
You'll learn about these methods in the <<converting-documents,converting documents sections>>.
...
Each part will than contain of title (ie. "Section one") and converted text content as html.
...
In the last case, the caller is the owner of the lifeycle of the object.
...
AsciidoctorJ bundles the [app]_asciidoctor-epub3_ gem inside the distribution JAR, which means that you can set the `backend` attribute to `epub3`.
...
// I thought we'd moved to Java 7?

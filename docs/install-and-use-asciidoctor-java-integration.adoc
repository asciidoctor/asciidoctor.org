= Getting Started with Asciidoctor in Java using AsciidoctorJ
...
:dagger: &#8224;
// URIs:
:uri-asciidoctor: http://asciidoctor.org/docs/what-is-asciidoctor
:uri-repo: http://github.com/asciidoctor/asciidoctorj
:uri-issues: https://github.com/asciidoctor/asciidoctorj/issues
...
:uri-bintray-artifact-query: https://bintray.com/lordofthejars/maven/asciidoctorj/view/general
:uri-bintray-artifact-detail: https://bintray.com/lordofthejars/maven/asciidoctorj/{artifact-version}/view
:uri-bintray-artifact-file: http://dl.bintray.com/lordofthejars/maven/org/asciidoctor/asciidoctorj/{artifact-version}/asciidoctorj-{artifact-version}
:uri-jruby-startup: http://github.com/jruby/jruby/wiki/Improving-startup-time
:uri-docs: link:/docs
:uri-maven-guide: link:/docs/install-and-use-asciidoctor-maven-plugin
:uri-gradle-guide: link:/docs/install-and-use-asciidoctor-gradle-plugin
...
The artifact information is summarized in the tables below.
...
|{uri-maven-artifact-detail}[{artifact-version}]{empty}^{dagger}^
|{uri-maven-artifact-file}.pom[pom] {uri-maven-artifact-file}.jar[jar] {uri-maven-artifact-file}-javadoc.jar[javadoc (jar)] {uri-maven-artifact-file}-sources.jar[source (jar)]
|===

[cols="2,4,^1,5"]
.Artifact information AsciidoctorJ in Bintray
|===
|Group Id |Artifact Id |Version |Download

|org.asciidoctor
|{uri-bintray-artifact-query}[asciidoctorj]
|{uri-bintray-artifact-detail}[{artifact-version}]{empty}^{dagger}^
|{uri-bintray-artifact-file}.pom[pom] {uri-bintray-artifact-file}.jar[jar] {uri-bintray-artifact-file}-javadoc.jar[javadoc (jar)] {uri-bintray-artifact-file}-sources.jar[source (jar)]
|===

{dagger} Based on Asciidoctor {artifact-version}.

== Installation

AsciidoctorJ is a standard `.jar` file.
To start using it, all you need to do is add the library to your project's classpath.
...
.Declaring the dependency in Maven's pom.xml
----
<dependencies>
  <dependency>
...
[source,groovy]
[subs="specialcharacters,attributes,callouts"]
.Declaring the dependency in Gradle's build.gradle
----
dependencies {
...
----
<1> Specifying the version of AsciidoctorJ implicitly selects the version of Asciidoctor core (i.e., Ruby).

// DA: Should we mention how to download if you just want to use the asciidoctorj command?

[TIP]
====
In addition to using AsciidoctorJ directly, you can invoke it as part of your build using the plugin for either Maven or Gradle.
To learn more about these plugins, refer to the following guides.
...
You'll learn more these methods in the next section, <<Converting documents>>.
...
NOTE: All the methods listed in Table 3 are overloaded to accomodate various input types and options.
...
The `convertFile` method will convert the content of an AsciiDoc file.
...
|Converts all files of given folder and all its subfolders. Also ignores files starting with underscore (_).
...
// DA: I think this section should come after "Convert options"
...
You can take two approaches to read reading the structure of an AsciiDoc document with AsciidoctorJ:

Using wrapper classes not connected with Ruby internal model::
This means that the structured is copied in Java non-proxied classes so any change does not modify the original document.

Using JRuby Java wrapper classes::
This way Java classes are linked to Ruby internal classes.
Any modification done here is reflected to the original document.
...
----
<1> converts all files in `docs` folder and its subfolders.

But also you can load the document inside a `Document` object.
This object represents the whole document not only the headers, and you may use it to navigate through the internals of a parsed document.
To load a document you can use `load` or `loadFile` methods.

The `readDocumentStructure` method provides easy and useful way of parsing asciidoc file into
the structured object. First of all it gathers exactly the same information as
`readDocumentHeader` and puts it in `header` field of `StructuredDocument` object.
...
converted text content as html.
...
Currently there are support for three kind of blocks.
...
But also blocks can be retrieved from query using `findBy` method.
...
----
<1> To make queries you need to use a `map` approach. Currently this is because of Asciidoctor API but it will change in near future.
<2> In this example all blocks with context as image is returned. Notice that (:) must be add in the value part.
...
`convert` method is overloaded so `org.asciidoctor.Options` can be passed instead of a `java.util.Map`.
...
Since Asciidoctor 0.1.3, attributes can be specified as `String` or `Array` instead of pair key/value by using `org.asciidoctor.Attributes.setAttributes(String)` or [x-]`org.asciidoctor.Attributes.setAttributes(String...)` and `AttributesBuilder` methods.
...
{uri-repo}[AsciidoctorJ] allows us to write extensions in _Java_ instead of _Ruby_.

In Asciidoctor, there are 7 kinds of extension points.
...
and second one register it using `JavaExtensionRegistry` class.
...
NOTE: First two cases, lifecycle of the instance is managed by JRuby.
In last case caller is the owner of the lifeycle of the object.

TIP: Using an already created instance as extension is really useful in cases of using CDI. You can for example inject any extension inside code, and then register that instance as extension.

==== Preprocessor

This extension updates an attribute value defined in document
...
<2> A constructor receiving a `Map` must be provided in case you want to send options to preprocessor.
<3> `process` method receives a `Document` and `PreprocessorReader`.
...
You can unregister all extensions by calling `org.asciidoctor.Asciidoctor.unregisterAllExtensions()` method.
...
<2> Ruby file containing a class implementing a Block extension is loaded inside Ruby runtime. Then block is registered with a name (rubyyell), and we pass the name of the class to be instantiated.
...
In previous examples extensions were registered manually in our code.
But there is another way in AsciidoctorJ to register extensions.
Since AsciidoctorJ 1.5.0 and SPI interface has been created.
This means that any implementation of this interface that is present on classpath will be executed.
...
<2> AsciidoctorJ will automatically run `register` method which will be the responsible of registering all extensions.
<3> All required (Java) extensions are registered.

And then you need to create a file called `org.asciidoctor.extension.spi.ExtensionRegistry` inside `META-INF/services` with the implementation full qualified name.
...
And that's all.
Now when a `.jar` file containing previous structure is dropped inside classpath of AsciidoctorJ, the `register` method will be executed automatically and the extensions will be registered.

=== Converting to EPUB3 (and KF8/MOBI)

AsciidoctorJ bundles the [app]_asciidoctor-epub3_ gem inside the distribution `.jar`, which means that you can set `backend` property to `epub3`, and the document will be converted to the http://idpf.org/epub/30[EPUB3^] format.

CAUTION: The [app]_asciidoctor-epub3_ gem is currently alpha.
...
Let's see an example of how to use AsciidoctorJ with the `epub3` backend.
...
----
<1> The EPUB3 converter currently requires the value of the `imagesdir` attribute to be `images`.
<2> The EPUB3 converter currently must be run on a “spine” document that has at least one include directive (and no other body content) in order to function properly.
...
----
<1> The EPUB3 converter must currently be run in `SAFE` or `UNSAFE` mode only due to a bug
<2> `epub3` is the name of the backed that must be set to convert to EPUB3.

=== Loading Ruby libraries

Simple extensions may be fully implemented in _Java_, but if you want to create a more complex extension you may write some polyglot extension mixed with Ruby and Java code.
This means that you may need to execute a _Ruby_ file or a _RubyGem_ (i.e., gem) inside your extension.

To load a _Ruby_ file inside the _Ruby_ runtime, you can use `org.asciidoctor.internal.RubyUtils.loadRubyClass(Ruby, InputStream)`.

You can also load a gem using an API that wraps Ruby's `require` command.
The gem must be already available inside classpath.
You then run `org.asciidoctor.internal.RubyUtils.requireLibrary(Ruby, String)`, passing the name of the gem as the second argument.

== Optimization

Sometimes JRuby starts slower than expected versus the C-based Ruby implementation (MRI).
Fortunately, JRuby offers flags that can be used to improve the start time and generally tune applications when running on JRuby.
...
Both flags are set by default inside the AsciidoctorJ integration project.
...
You can find a full explanation on how to improve the start time of JRuby applications at <<Optimization>>.

== Running AsciidoctorJ on WildFly AS

If you want to use AsciidoctorJ in your application deployed on [app]_WilFly AS_, you have to follow the instruction below:
...
** create the module descriptor file [path]_module.xml_ like this:
+
[source,xml]
...
** you can either add the dependency just into the [path]_MANIFEST.MF_ file.
+
 ...
 ** _or_ you can configure the dependency into the [path]_pom.xml_ thanks to the [app]_Maven JAR/WAR plugin_.
+
[source, xml]
[subs="specialcharacters,attributes,callouts"]
...
<1> The AsciidoctorJ dependency and all these transitives dependencies don't need to be added to the final WAR since all JARs are available through the module.

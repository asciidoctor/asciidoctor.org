...
If you're refering to a content element other than a section, you'll need to define an anchor on that element explicitly.
...
Guard::AsciiDoc is a plugin for https://github.com/guard/guard[Guard] that converts watched AsciiDoc files to the specified output format whenever they change using Asciidoctor.
...
|:backend: deckjs
|Activates the deck.js backend to convert the document as a deck.js presentation
|:backend: deckjs
...
To convert your presentation as HTML5, execute the command:

...
A new cell begins when a vertical bar (+|+) is encountered.
...
When a single number is assigned to the +cols+ attribute, that its value indicates the number of columns.
...
Let's apply the header style to the first column, the monospaced style to the second, and the strong style to the third.
...
.2+|Content in a single cell that spans rows 1 and 2
|Cell in column 2, row 1
|Cell in column 3, row 1

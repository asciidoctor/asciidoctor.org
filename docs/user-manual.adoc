...
The AsciiDoc syntax provides a number of ways to customize a document's Table of Contents (ToC).
...
You can apply the AsciiDoc style to an individual cell by prefixing the vertical bar with an +a+:
...
Tables can also be created directly from CSV or DSV data.
Just set the +format+ block attribute to +csv+ or +dsv+ and insert the data inside the block delimiters, either directly:

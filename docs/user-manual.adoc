...
fopub Tool::
  Directions for using the tool are documented in the {fopub-doc-ref}[asciidoctor-fopub README]. To alter themes, it is necessary to pass XSL parameters or modify the XSLT.
asciidoctor-pdf Tool::
  Directions for using the tool are documented in the {asciidoctor-pdf-docs-ref}[asciidoctor-pdf README]. There is built-in theming via YAML, which is documented in  http://gist.asciidoctor.org/?github-asciidoctor%2Fasciidoctor-pdf%2F%2Fdocs%2Ftheming-guide.adoc[this gist].

...
----
<1> The `--gem-path <path>` option tells Asciidoctor's JRuby runtime where to find gems when using `--require`. 
This should typically be the same as your `$GEM_PATH` environment variable.

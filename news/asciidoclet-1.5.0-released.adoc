...
== What is Asciidoclet?
{link-guide}[Asciidoclet] allows Java developers to write formatted Javadoc comments using http://asciidoc.org[AsciiDoc] syntax instead of HTML.
...
Ruby libraries (Gems) are loaded using the `-r`/`--require` option. For example, `-r asciidoctor-diagram` loads the (excellent!) {uri-diagram}[asciidoctor-diagram] gem, enabling plain text diagrams directly in the javadoc!

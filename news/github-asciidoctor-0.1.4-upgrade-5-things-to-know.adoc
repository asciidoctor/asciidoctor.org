...
What makes things even more difficult is that the content is passed through an HTML sanitizer that strips away most of the HTML attributes, so even if we could map to existing styles on GitHub, we can't.
In a future upgrade, we'll work on finding a way to make these patterns, such as admonition blocks.
--
...
The recent Asciidoctor upgrade on GitHub certainly brought a lot of improvements.

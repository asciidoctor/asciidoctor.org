...
*87 issues* were closed along the way, resulting in *310 commits*.
...
In this case, the text will still be formatted, but the attribute list will unescaped and output verbatim. {issue-ref}/421[#421]
...
The goal for Asciidoctor has always been to allow extensions to be written using the full power of a programming language (whether it be Ruby, Java, Groovy or JavaScript), similar to what we've done with backend (rendering) mechanism.
...
Block macro processor::
  Registers a custom block macro and process it (e.g., `gist::12345[]`).

Inline macro processor::
  Registers a custom inline macro and process it (e.g., `btn:[Save]`).
...
This change was made since the automatic numbering of an ordered list doesn't align with the numbering strategy in AsciiDoc and is therefore semantically incorrect.
This also eliminates the "double numbering of sections" problem when the default stylesheet is absent and the work-around of using +type="none"+ on the list to address this problem.
...
Previously, Asciidoctor would increment the section number counter in regions of the document where section numbering is disabled.
This resulted in section numbers being skipped.
Asciidoctor now freezes the counter where numbering is surpressed to prevent gaps in the numbering.
...
* The TOC can be positioned to the right by assigning the value +right+ to the +toc-position+ or +toc2+ attribute. {issue-ref}/467[#467]
...
* Don't prepend +imagesdir+ to image target is a URI or absolute path

...
.What is Asciidoctor?
****
Asciidoctor is a fast, open source text processor and publishing toolchain written in Ruby for transforming AsciiDoc markup into HTML 5, DocBook 5.0 or 4.5 and other custom formats.
...
*Eighty-seven issues* were closed along the way, resulting in *310 commits*.
...
However, what we've found is that new users often don't discover the default stylesheet and are confused when many of the features that rely on CSS do not work as expected.
...
Readers shouldn't have to understand why or worry about the documentation is putting extra characters in the clipboard.
Copy and paste should just work.

In this release, we used some CSS ninja moves to prevent the callouts from being selected.
No matter how hard the reader tries, those callouts won't get selected.
...
TIP: This syntax also works for placing callouts in HTML listings.
...
It turns out, the problem is that the very presence of the callouts in the code, whether behind line comments or not, causes them to get caught in the highlighting and mangled.
...
It works when using CodeRay or Pygments.
...
That's right.
Asciidoctor now lets you use Pygments for source code highlighting!
...
To apply the +nowrap+ option globally, set the +prewrap+ attribute on the document.
...
Now you can make more of them, faster, with Asciidoctor.

=== Getting Things Done using checklists

If you use Asciidoctor to track the completion of tasks, get ready to start checking things off your list!
List items can now be marked complete using checklists.
...
Since HTML generated from Asciidoctor is usually static, the checkbox is set as disabled to make it appear as a simple mark.
...
_Resolves issues {issue-ref}/364[#364] and {issue-ref}/472[#472]._ 
...
The file extension is set using the +icontype+ attribute, which defaults to +png+.
...
Ever since I began working on Asciidoctor, I've been eagerly awaiting the chance to work on the extensions API.
...
Just keep in mind that you may need to rework parts of your extension when you upgrade Asciidoctor.
====
...
That way, you don't have to do yak shaving to get the functionality you want, and you can distribute the extension using defacto-standard packaging mechanisms (like RubyGems or JARs).
...
Postprocessor::
  Processes the output after the document has been rendered, but before it's written to file.
...
This change was made since the automatic numbering of an ordered list doesn't align with the numbering strategy in AsciiDoc and therefore is semantically incorrect.
This also eliminates the "double numbering of sections" problem that occurred when the default stylesheet was absent.
...
* The TOC can be positioned to the right by assigning the value +right+ to the +toc-position+ or +toc2+ attribute. _{issue-ref}/467[#467]_

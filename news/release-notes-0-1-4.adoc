...
The Asciidoctor processor is accessible to any JVM language or build system via the {java-int-ref}[Java integration library].
...
*Eighty-seven issues* -- that's right, _87_ -- were closed along the way, resulting in *310 commits*.
...
(Or jump to <<tl-dr>> to get the abbreviated version.)
...
The focus of this change is to ensure callouts are an aid, not a pain.
...
All we have to do is stretch out the angled brackets around the callout number to make the callout an XML comment.
...
* set the +prewrap+ document attribute
...
If you use Asciidoctor to track the completion of tasks, get ready to start checking those tasks off!

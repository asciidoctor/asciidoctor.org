...
It's been exactly 6 months since I (Dan) picked up Asciidoctor and started driving it forward.
The project sure has come a long way since then.
With this release, I think Asciidoctor has become the center point of the AsciiDoc community.
This post should help to demonstrate that fact.
...
Here some other things to know about this notation:
...
Asciidoctor now supports Markdown-style blockquotes:
...
The default stylesheet has been updated accordingly.
It has also been updated with some new styles for the experimental user input macros.
...
Now there are even more options to pull source code snippets into your document and highlight them.
...
Asciidoctor will link to the prettify JavaScript library and stylesheet and emit HTML that prettify requires to highlight the source code.
...
Asciidoctor will emit the following HTML:
...
That's right!
Asciidoctor now supports three key elements from the Markdown syntax (where AsciiDoc and Markdown don't already overlap).
...
Support for Markdown-style headings and blockquotes join support for fenced code blocks (from GitHub-flavored Markdown), added in a previous release.
...
The +partintro+ and +abstract+ styles can now be used on open blocks and are handled correctly by the renderer.
...
Previously, a block title line above the level-0 heading was being processed and passed on to first content block. 
AsciiDoc sees the block title as the first line of content and does not create a header as a result. 
Asciidoc's behavior is now consistent with AsciiDoc.
...
This parallels the commandline call:
...
Additional thanks goes to everyone else who has contributed to the project.
